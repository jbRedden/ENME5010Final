...
Veqn2::Veqn2() {}
Veqn2::Veqn2(StaggeredGrid *grid, double rho, double mu, 
...
void Veqn2::InitField(double v0) {
...
double** Veqn2::GetVStar() {
    return vStar;
}

void Veqn2::UpdateCoefficients(Ueqn2 &uEqn, Peqn2 &pEqn) {
    ...
    for (size_t j = 1; j < ny-1; j++) {
        for (size_t i = 1; i < nx-1; i++) {
            ...
            double dx = cell.GetDX();
            double dy = cell.GetDY();
            A[i-1][j-1] = 2*rho*dx*dy/dt + rho*dx/4 *(v[i][j+1]-v[i][j-1]) + rho*dy/4 * (u[i+1][j-1] + u[i+1][j] - u[i][j] - u[i][j-1]) + 2 * mu * dx / dy + 2 * mu * dy/dx; 
            B[i-1][j-1] = rho*dy/4*(u[i+1][j-1] + u[i+1][j]) - mu * dy/dx;
            C[i-1][j-1] = -rho*dy/4 * (u[i][j]+u[i][j-1]) - mu * dy/dx;

            D[i-1][j-1] = rho*dx/4 * (v[i][j] + v[i][j+1]) - mu * dx/dy;
            E[i-1][j-1] = -rho*dx/4 * (v[i][j] + v[i][j-1]) - mu * dx/dy;
            F[i-1][j-1] = 2 * rho * dx * dy / dt * v[i][j];
            */
        }
    }
...
void Veqn2::Solve(Peqn2 &pEqn) {
    ...
    for (size_t i = 0; i < nx2; i++) {
        ...
        for (size_t j = 0; j < ny2; j++) vStar[i+1][j+1] = f[j];
    }
...
void Veqn2::ThomasAlg(double *a, double *b, double *c, double *d, double *f, size_t n) {
